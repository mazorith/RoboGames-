...
PI_THREAD(rotateScan); //CHANGE TO PI_THREAD IF STILL VOID* (name of thread for all before running)
PI_THREAD(rotateTo);
PI_THREAD(moveDirection);
PI_THREAD(returnMove);
PI_THREAD(timedMove);
//================================================
	...
	while (true)
	{
		...
		else 
		{
			...
			if (dumpPhase)
			{
				...
				timeToMove = 1; //[time it takes to move to goal where tape is visible]);
				...
				piThreadCreate(timedMove);
				timeToMove = 1; //[time it takes to finish moving to goal];
				delay(timetoMove * 1000);
				...
				piThreadCreate(timedMove);
				timeToMove = 1; //[time it takes to finish moving];
				delay(timetoMove * 1000);
